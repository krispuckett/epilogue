...
struct AmbientBookView: View {
    ...
    @Binding var textColor: Color  // Add this binding
    ...
    var body: some View {
        ZStack {
            if let palette = palette, let image = coverImage {
                ...
                Image(uiImage: image)
                    ...
                    .blur(radius: 120)
                    .opacity(palette.luminance > 0.7 ? 0.25 : 0.45)  // Reduced from 0.35 and 0.6
        ...
        .onChange(of: palette) { newPalette in
            if let palette = newPalette {
                ...
                textColor = palette.luminance > 0.6 ? .black : .white
            }
        }
    }
...
struct GradientOrb: View {
    ...
    private var opacity: Double {
        isMainColor ? 0.85 : 0.65  // Increased from 0.7 and 0.5
    }
