...
struct BookDetailView: View {
    ...
    // Computed colors from display scheme (single source of truth)
    private var textColor: Color {
        displayScheme?.primaryTextColor ?? .white
    }
    
    private var secondaryTextColor: Color {
        displayScheme?.secondaryTextColor ?? .white.opacity(0.8)
    }
    
    private var accentColor: Color {
        displayScheme?.accentTextColor ?? (colorPalette?.accent ?? .orange)
    }
    
    private var textShadowColor: Color {
        displayScheme?.textShadowColor ?? .black.opacity(0.3)
    }
    ...
    private var centeredHeaderView: some View {
        VStack(spacing: 16) {
            ...
            Text(book.title)
                ...
                #if DEBUG
                .modifier(ContrastValidator(
                    textColor: textColor,
                    backgroundColor: displayScheme?.averageGradientColor ?? .gray
                ))
                #endif
            ...
            if description.count > 200 {
                Button {
                    ...
                    Text(summaryExpanded ? "Read less" : "Read more")
                        .font(.caption)
                        .foregroundColor(accentColor)
                        .shadow(color: textShadowColor.opacity(0.7), radius: 0.5, x: 0, y: 0.5)
                }
            }
        }
        .padding(20)
