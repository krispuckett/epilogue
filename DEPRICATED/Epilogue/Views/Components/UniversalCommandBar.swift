...
struct InlineNoteComposerView: View {
    ...
    private func saveNote() {
        ...
        let finalAuthor: String? = selectedBook?.author
        let finalBookTitle: String? = selectedBook?.title
        let bookId: UUID? = selectedBook?.localId
...
// LiteraryQuote and LiteraryQuotes are now imported from Models/LiteraryQuotes.swift

struct InlineLiteraryLoadingView: View {
    let message: String?
...
struct InlineReadingProgressIndicator: View {
    let currentPage: Int
    let totalPages: Int
    let width: CGFloat
...
// Book.currentPage extension is defined in Views/Library/ReadingProgressIndicator.swift
...
struct UniversalCommandBar: View {
    ...
    // Note: showNoteComposer removed - using composerMode instead
    ...
    var body: some View {
            ...
            if isExpanded {
                ...
                HStack(spacing: 12) {
                    ...
                    HStack(spacing: 0) {
                        ...
                        TabBarItem(
                            customIcon: FeatherIcon(size: 18),
                            label: "Notes",
        ...
        // Note composition is handled by composerMode and InlineComposerView
    ...
    private func saveNote(_ text: String) {
        ...
        // Note saved - composer mode already handles the UI
        isFocused = false
    }
...
// MARK: - Tab Bar Item
struct TabBarItem<IconView: View>: View {
    let icon: String?
    let customIcon: IconView?
    let label: String
    let isSelected: Bool
